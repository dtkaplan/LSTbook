---
title: "Simulations with Directed Acyclic Graphs"
...
```{r echo=FALSE}
Legend2 <- dag_make(
  season ~ categorical(c("Spring", "Summer", "Fall", "Winter")),
  drowning ~ pois(rate=.05 + .15*(season=="Summer")),
  ice_cream ~ signif(unif(1,3) + (season=="Summer")*unif(2,5),2)
)
```
...
Legend <- dag_make(
  season ~ categorical(c("Spring", "Summer", "Fall", "Winter")),
  ice_cream ~ signif(unif(1,3) + (season=="Summer")*unif(2,5),2),
  drowning ~ pois(rate=ice_cream/30 + .5*(season=="Summer"))
)
```
