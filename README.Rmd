...
In the examples that follow, we will use the `{LST}` function `pointplot()` which handles both numerical and categorical variables using one syntax. Here's a graphic for looking at the difference between two means. 
...
```{r echo = FALSE}
Galton |> pointplot(height ~ sex, annot = "model", model_ink = 1)
```
...
```{r echo=FALSE}
Whickham |> pointplot(outcome ~ smoker, annot = "model", model_ink =1, point_ink=.2)
```
